...
ORDER = ['중학교수학', '공통수학1', '공통수학2', '대수', '미적분Ⅰ', '미적분Ⅱ', '확통', '기하',
         '교육과정 밖']
# «교육과정 밖» — 지금 배우지 않는 내용. 부등식의 영역(옛 고1)이 대표다.
# 낱말로는 못 찾아서 손입력 표로만 들어온다.

# 설문(교과목 고르기)에서 사람이 고른 값 → 여기 이름
FROM_SURVEY = {'중학교': '중학교수학', '고1': '공통수학1', '대수': '대수',
...
def scope_of(e):
    ...
    if g == '고1':
        ...
        return ['중학교수학'] if r == '3월' else ['공통수학1', '공통수학2']
    ...
    if s == '가형':
        return ['대수', '미적분Ⅰ', '미적분Ⅱ', '확통', '기하', '공통수학1', '공통수학2']
    if s == '나형':
        return ['대수', '미적분Ⅰ', '미적분Ⅱ', '확통', '공통수학1', '공통수학2']
    if s == '공통':
        return ['대수', '미적분Ⅰ']
    return list(ORDER)
...
RULES = [
 ...
 # 공통수학1 보다 먼저 본다. «부등식» 처럼 넓은 말이 먼저 집어 가면 안 된다.
 ('공통수학2', r'직선의방정식|원의방정식|평행이동|대칭이동|내분|외분|자취|좌표평면'
              r'|집합의연산|부분집합|공집합|합집합|교집합|여집합|원소의개수'
              r'|명제|필요조건|충분조건|참인명제|대우|진리집합|조건의부정'
              r'|합성함수|역함수|유리함수|무리함수|일대일대응|일대일함수|정의역|치역|점근선'
              # 도형의 방정식 — 원·직선을 다루는 말들
              r'|점과직선사이의거리|원과직선|두직선의|위치관계|접하는원|절편'),
 # 공통수학1 — 다항식 · 방정식과 부등식 · 경우의 수 · 행렬.
 # 순열·조합은 확통에도 있지만, 확통 표가 위에 있으니 확통이 열린 시험에서는
 # 그쪽이 먼저 집어 간다. 고1 시험에서만 여기로 온다.
 ('공통수학1', r'다항식|인수분해|나머지정리|항등식|복소수|허수|이차방정식|이차함수|판별식'
              r'|삼차방정식|사차방정식|연립|부등식|절댓값|행렬|근과계수|곱셈공식'
              r'|순열|조합|경우의수|합의법칙|곱의법칙|약수|배수'
              r'|산술평균|기하평균|나눗셈|음수의제곱근|허수단위'),
]
...
SIGNS = [
 ...
 ('공통수학2', r'부분집합|∈|⊂|∅|명제|대우|역함수|합성함수|유리함수|무리함수|점근선'
              r'|내분|외분|원의방정식|두집합|전체집합|집합의연산|원소의합|n\(A|∘'
              r'|f\^\(-1\)|f-1\(|f:X→|X에서X|일대일대응|일대일함수'
              r'|정사각형|직사각형|정육면체|겉넓이|서로수직|두직선|원x\^2|선분AB|자취'
              r'|수직인직선|평행한직선|평행할때|접할때|절편|사이의거리|반지름의길이가'),
 ('공통수학1', r'허수|i=√|판별식|인수분해|나머지|행렬|이차함수|이차방정식'
              r'|최솟값은\?|최댓값은\?|나눈몫|자연수p,q|부피는'),
]
...
# 문제 글을 볼 때는 두 표를 «과목별로» 합쳐서 본다.
# 낱말 표를 통째로 먼저 돌리면, 아래쪽 과목의 낱말이 위쪽 과목의 기호를 이긴다.
# 실제로 «…의 역함수를 g(x)라 하자. 4f′(x)+…» 문항이 공통수학2로 갔다.
# 낱말 «역함수»가 기호 «f′» 보다 먼저 걸렸기 때문이다.
def _merge():
    R = dict(RULES); S = dict(SIGNS)
    out = []
    for name, pat in RULES:
        if name in S:
            out.append((name, '(?:' + pat + ')|(?:' + S[name] + ')'))
        else:
            out.append((name, pat))
    for name, pat in SIGNS:          # 낱말 표에 없는 과목이 있으면 뒤에 붙인다
        if name not in R:
            out.append((name, pat))
    return out
...
def guess(text, allow, signs=False):
    ...
    signs=False — 해설의 «출제의도» 한 마디. 낱말 표만 본다.
    signs=True  — 문제 글. 낱말과 기호를 과목별로 합친 표를 본다."""
    if not text:
        return None
    t = re.sub(r'\s', '', text)
    for name, pat in (BOTH if signs else RULES):
