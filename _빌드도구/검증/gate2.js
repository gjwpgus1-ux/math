...
(async()=>{
  ...
  let H=boot(null,{gate:true}); await wait(180);
  ok('비교부터', open(H,'cmp'));
  ...
  console.log('\n[2] 성취기준으로 답해도 통과한다');
  let G=boot(null,{gate:true}); await wait(180);
  ...
  let K=boot(null,{gate:true}); await wait(180);
  ...
  console.log('\n[5] 건너뛰기는 답으로 치지 않는다');
  let J=boot(null,{gate:true}); await wait(180);
