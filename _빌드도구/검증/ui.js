...
(async()=>{
  ...
  ok('질문 글씨 15px', css(/\.crail \.rq\{[^}]*font-size:15px/));
  ...
  ok('질문 글씨 17px', css(/#std \.stop \.rq\{[^}]*font-size:17px/));

  console.log('\n[3] 성취기준 글씨와 후보 수');
  ok('본문 15px', css(/\.scand \.bd\{[^}]*font-size:14px/));
  ok('카드 자체도 14px', css(/\.scand\{[^}]*font-size:14px/));
  const cands=[...$('stdCands').querySelectorAll('.scand')];
  ok('후보 4개 이하', cands.length<=4 && cands.length>0, cands.length);
  ok('코드는 작게 11.5px', css(/\.scand \.cd\{[^}]*font-size:11.5px/));
  ...
  ok('검색 결과도 큰 글씨', css(/\.scand \.bd\{[^}]*font-size:14px/));
